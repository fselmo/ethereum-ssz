...
class TestComplexVectors:
    ...
    def test_long_byte_vector_48(self):
        ...
        htr = hash_tree_root(vec)
        expected_htr = Bytes32(h(
            bytes(range(32)),
            bytes(range(32, 48)) + b'\x00' * 16
        ))
        assert htr == expected_htr
...
class TestComplexLists:
    ...
    def test_uint16_list(self):
        ...
        lst = SSZList([U16(0xaabb), U16(0xc0ad), U16(0xeeff)],
                      max_length=32, element_type=U16)
    ...
    def test_uint32_list(self):
        ...
        lst = SSZList([U32(0xaabb), U32(0xc0ad), U32(0xeeff)],
                      max_length=128, element_type=U32)
        encoded = ssz.encode(lst)
    ...
    def test_byte_list_partial_256(self):
        ...
        htr = hash_tree_root(lst)
        expected_htr = Bytes32(h(
            h(h(h(chunk(bytes(range(6))), zero_hashes[0]),
                zero_hashes[1]),
              zero_hashes[2]),
            chunk(b'\x06')
        ))
...
class TestUint256Lists:
    ...
    def test_uint256_list_small(self):
        ...
        lst = SSZList([
            ...
            U256(0xc0ad),
            U256(0xeeff)
        ], max_length=32, element_type=U256)
    ...
    def test_uint256_list_long(self):
        ...
        lst = SSZList([U256(i) for i in range(1, 20)],
                      max_length=128, element_type=U256)

        encoded = ssz.encode(lst)

        # Verify encoding of each element
        expected = b''.join(
            i.to_bytes(32, 'little') for i in range(1, 20)
        )
        assert encoded == expected
